...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, outputs):
        ...
        self.head = nn.Sequential(nn.Linear(392, 256),
                                  nn.SELU(),
                                  nn.Linear(256, 128),
                                  nn.SELU(),
...
def get_img(world_state,agent_obj,dirct,device):
    ...
    speed = torch.zeros(1, 1, 21, 21).to(device) + agent_obj.current_speed
    ts = torch.zeros(1, 1, 21, 21).to(device) + agent_obj.current_turning_speed
    ...
    while world_state.is_mission_running:
        ...
        if world_state.number_of_observations_since_last_state > 0:
            ...
            if last_dis > distance:
                current_r += min((50 - distance)*0.3,1)*128
                dirct = dirct*0
